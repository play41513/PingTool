...
bool Port_test::ChangeLanSpeed(AnsiString VPID, AnsiString GUID,
	...
	{
		frmMain->Height = 289;
		frmMain->plErrMsg->Caption = "(!)搜尋不到DHCP Server IP";
		frmMain->pan_1000->Color = clRed;
		return false;
	}
	//
	if(!cmd_ping(ip,PingIP,1000))
	{
		frmMain->Height = 289;
		frmMain->pan_1000->Color = clRed;
		return false;
...
bool Port_test::cmd_ping(AnsiString DevIP,AnsiString PingIP,int speed) { // fping
	...
	if (CallCommand_C("ping " + PingIP + " -S "+DevIP+" -l 65500 -n 3 \> pingmsg.log")) {//20170816 jim 為了讓88772 10M 100M 在背景ping 同時測試會衝突問題而改
...
AnsiString Port_test::getDHCP_Server_IP(AnsiString LANname) { // 得到DHCP_SERVER IP
	...
	if (ERROR_SUCCESS == GetAdaptersInfo((IP_ADAPTER_INFO*)pbBuf1, &ulLen1)) {
		...
		do {
			...
			if (PORT->lan_find_inf(LanName.c_str(), LANname.c_str())) {
				delete pbBuf1;
				return (AnsiString)pipai->DhcpServer.IpAddress.String;
			}
			pipai = pipai->Next;
			IpCount++;
		}
