...
bool Port_test::USB_RT(AnsiString filename) { // USB讀寫檔案比對
	bool Result = true;
	AnsiString tempN;
	for (char i = 'A'; i < 'Z'; i++) {
		...
		if (Ufile.is_open() && Lfile.is_open()) {
			...
			while (!Ufile.eof() && !Lfile.eof()) {
				...
				if (Ufilemsg == Lfilemsg) {
					...
					fstream fp;
					fp.open(tempN.c_str(), ios::out); // 開啟檔案
					if(!fp)
						return false;
...
bool Port_test::ChangeLanSpeed(AnsiString VPID, AnsiString GUID,
	...
	frmMain->pan_1000->Color = clGreen;   //20161103 jim加燈號
	if(!frmMain->TestResult)
			return false;
	Search_System_Exist_Lan_Speed(GUID, VPID, LANname, "4");  //切速度 100M
	if (!cmd_restartdevice(VPID))
		return false;
	pingnum = 0;
	ip="";
	...
	frmMain->pan_100->Color = clGreen;
		if(!frmMain->TestResult)
				return false;
		Search_System_Exist_Lan_Speed(GUID, VPID, LANname, "2");  //切速度 10M
		if (!cmd_restartdevice(VPID))
			return false;
		pingnum = 0;
		ip="";
		...
		Search_System_Exist_Lan_Speed(GUID, VPID, LANname, "6");  //切速度 1000M
	return true;
}
...
bool Port_test::ChangeLanSpeed2(AnsiString VPID, AnsiString GUID,
	...
	while(!lan_find_inf(ip.c_str(), "192.168")){
		...
		ip = getLanIP(LANname);
		if(pingnum == 100){
			frmMain->Height = 289;   //20161130 jim 加顯示錯誤訊息
			frmMain->plErrMsg->Caption = "無法配到IP...";
			frmMain->pan_100->Color = clRed;  //20161103 jim加燈號
			return false;
		}
	}
	// 得到ping ip
	...
	frmMain->pan_100->Color = clGreen;   //20161103 jim加燈號
	if(!frmMain->TestResult)
			return false;
	Search_System_Exist_Lan_Speed(GUID, VPID, LANname, "2");  //切速度 10M
	if (!cmd_restartdevice(VPID))
...
bool Port_test::Fping(AnsiString PingIP) { // fping
	...
	if (CallCommand_C("ping " + PingIP + " -n 3 \> pingmsg.log")) {//20170816 jim 為了讓88772 10M 100M 在背景ping 同時測試會衝突問題而改
...
AnsiString Port_test::GetLanName(AnsiString PVID){ // 由VIDPID取得網卡名稱
