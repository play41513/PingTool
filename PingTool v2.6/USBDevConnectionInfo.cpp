...
DWORD USBDevConnectionInfo::GetUSBType(AnsiString USBInfo,TMemo* moDebug)
{
	TListUSBDEV_CONN_INFO->Clear();
	AnsiString VID = USBInfo.UpperCase().SubString(USBInfo.Pos("VID")+4,4).UpperCase();
	AnsiString PID = USBInfo.UpperCase().SubString(USBInfo.Pos("PID")+4,4).UpperCase();
	AnsiString Path;
	if(USBInfo.Pos("#"))
	{
		if(USBInfo.Pos("{"))
	...
	if(moDebug != NULL)
		moDebug->Lines->Add(TListUSBDEV_CONN_INFO->Text);
	AnsiString a;
...
bool USBDevConnectionInfo::RestartDev(AnsiString VID,AnsiString PID,AnsiString DevPath)
{
	HDEVINFO hDevInfo;
	SP_DEVINFO_DATA DeviceInfoData;
	DWORD i,j;
