...
//LOG(LS_SENSITIVE) << _T("this is an error.") << _T("這是一條錯誤信息");
//LOG(LS_WARNING) << _T("Create buffer fail! size= ") << IntToStr(bSize).c_str();
//MessageBox(Handle, _T("Memory insufficient"), _T("WARNING"), MB_ICONWARNING);
//LOG_INFO()<< _T("Initializeing buffer. size= ") << IntToStr(bSize).c_str();
...
//	locale loc = locale::global(locale("") ); // 这是C++的方式
	...
	AnsiString timemsg = FormatDateTime("yyyy/mm/dd hh:nn:ss", Now());
...
//		hFile_ = ::CreateFile(_T("loging.log"),GENERIC_WRITE,0,0,OPEN_ALWAYS,FILE_ATTRIBUTE_NORMAL,0);
		hFile_ = ::CreateFile(_T("TEST_TOOL\\loging.log"),GENERIC_WRITE,0,0,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,0);

		if(INVALID_HANDLE_VALUE == hFile_){
			hFile_ = ::CreateFile(_T("TEST_TOOL\\loging.log"),GENERIC_WRITE,0,0,OPEN_ALWAYS,FILE_ATTRIBUTE_NORMAL,0);
