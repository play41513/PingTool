...
bool USBDevConnectionInfo::GetDevUSB(AnsiString VID,AnsiString PID)
{
	...
	for(int j=0;j<TListUSBDEV_CONN_INFO->Count;j++)
	{
		...
		if (strstr(AnsiString(TListUSBDEV_CONN_INFO->Strings[j]).c_str(),("VID :"+VID).c_str()))//確認裝置VID
		{
			j++;
			b = TListUSBDEV_CONN_INFO->Strings[j];
			if (strstr(AnsiString(TListUSBDEV_CONN_INFO->Strings[j]).c_str(),("PID :"+PID).c_str()))//確認裝置PID
			{
...
{
	wchar_t buf[BUFSIZE]; // XXXXX How big does this have to be? Dynamically size it?
    ...
    while (!walkDone)
    {
		...
		if (cr == CR_SUCCESS && strcmp(DriverName, AnsiString(buf).c_str()) == 0)
		{
            ...
            //Device ID
			len = sizeof(AnsiString(buf).c_str());
			cr= CM_Get_Device_IDA(devInst, AnsiString(buf).c_str(), len, 0);
