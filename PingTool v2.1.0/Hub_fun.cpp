...
bool Port_test::ChangeLanSpeed(AnsiString VPID, AnsiString GUID,
	...
	{
		if(bUSB3)
			AddNewMAC(GUID, VPID, LANname, "6");
		else
			AddNewMAC(GUID, VPID, LANname, "4");
	...
	while(!ip.Pos("192.168")){
		...
		if(GetTickCount() >= timeout)
		{
			...
			frmMain->moDebug->Lines->Add("IP :"+ip);
	...
	while(!sList->DelimitedText.Pos("192.168"))
	{
		...
		if(GetTickCount() >= timeout)
		{
			frmMain->moDebug->Lines->Add("DHCP_IP :"+sList->DelimitedText);
			frmMain->ERROR_MSG = "(!)Not_Find_DHCPServer_IP";
			delete sList;
			return false;
		}
