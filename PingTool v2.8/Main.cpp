		...
		switch (Test_Step)
		{
			...
			case USB3_TEST:
				try
				{
					...
					DWORD dwResult = USBCONInfo->GetUSBType(g_DevVPID,moDebug);
					if(dwResult == USBTYPE_SUPER_SPEED)
					{
						bUSB3 = true;
						Test_Step = LAN_TEST;
					...
					else if(dwResult == USBTYPE_LOW_SPEED_PLUS)
					{   //取不到介面
...
void __fastcall TfrmMain::ckb2500MClick(TObject *Sender)
{
 	pl1000M->Caption = ckb2500M->Checked ? "2.5G":"1000M";
}
//---------------------------------------------------------------------------
